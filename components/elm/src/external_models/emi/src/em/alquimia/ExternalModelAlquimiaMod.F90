  ...
  do k=1,this%chem_sizes%num_primary
    ...
    if(this%is_dissolved_gas(k)) then
      ...
      do j=1,nlevdecomp
        ...
        !total_resist = total_resist + (diffus(j)+2.5e-9_r8*0.005_r8*exp(10.0_r8*sat(j)*liq_frac(j)*porosity(j)))*dzsoi_decomp(j) !BAM: this was Ben's surface equilib. fix, switching back to previous (above)
